...
function MenuSection({ menuItems }) {
	...
	const handleOrderedFood = (food) => {
		if (!orderHandled) {
			...
			if (
				cartItems.length > 0 &&
				Number(restaurant_id) !== Number(food.restaurant_id)
			) {
					...
					>
						...
						>
							...
							<Text ml={2}>
								Rs{" "}
								{Math.round(
									orderedFood?.price * orderedQuantity || 0
								)}
							</Text>
						</Button>
					</ModalFooter>
					...
					<ModalBody pb={6}>
						...
						<Box mt={3} mb={4}>
							<Text mb={3} fontWeight={400} fontSize={"18px"}>
								Your cart already has item from another
								restaurant. You have to clear cart items and add
								items from this restaurant. Do you want to
								proceed ?
							</Text>
